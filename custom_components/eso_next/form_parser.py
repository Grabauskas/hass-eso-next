...
class FormParser(HTMLParser):
    ...
    def handle_form_tag(self, attrs: tuple) -> None:
        attributes = dict(attrs)
        if "action" in attributes:
            _LOGGER.debug(f"Found form action: {attributes['action']}")
            self.form["action"] = attributes["action"]
